...
@WebServlet(name = "PubsDownloadServlet")
public class PubsDownloadServlet extends HttpServlet {
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        Properties pubs = new Properties();
        for (int i = 0; i < users.length; i++) {
            String username = users[i];
            String keyPath = allPath + "/" + username + "/key";
            Properties pub = Utils.getProperties(keyPath + "/pub.properties");
            String pubKey = pub.getProperty("pub");
            pubs.setProperty("u" + (i+1), username);
            pubs.setProperty("key"+(i+1), pubKey);
        }
        pubs.setProperty("user_num", String.valueOf(users.length));
        ...
        pubs.store(new FileOutputStream(downloadPath + "/pubs.properties"), "文件自述");
